...
grouped_df <- funcoes %>%
   ...
   select(-`Abrangência`) %>% 
   filter(`Função` == "Saúde" | `Função` == "Educação")
...
grouped_df1 <- funcoes %>%
   dplyr::group_by(`Função`) %>% 
   select(-`Abrangência`) %>% 
   filter(`Função` == "Lazer e esportes" | `Função` == "Comércio e serviços (gerais)" | `Função` == "Comércio e serviços (especializados)" | `Função` == "Postos de trabalho")
...
.bigger[
**Decisões metodológicas sobre acessibilidade**
]
...
.bigger[
Impedância e criticidade nas redes de modos ativos e tempos de deslocamento
]
...
.bigger[
**Categorização** de cada indicador em **classes** para análise posterior das regiões mais vulneráveis em relação à acessibilidade
]
...
<p style="text-align: center;"><span style="color: #2c8475;"><em>renataoliveira@gmail.com</em></span></p>

<p style="text-align: center;"><span style="color: #2c8475;"><em>lilianfontes@mail.uft.edu.br</em></span></p>
